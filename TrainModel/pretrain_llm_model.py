...
@FilePath: /RewriteLLM/TrainModel/pretrain_llm_model.py
@Description: 
"""
import os
...
from TrainModel.dataset.dataset import construct_dataset
...
parser.add_argument(
    ...
    default=None,
    type=str,
    help="Model config, when you specify ckpt, this param won't be used.",
)
...
def main():
    assert (
        args.model_config or config["train"]["ckpt"]
    ...
    if (
        ...
        or "Llama" in config["data"]["tokenizer_path"]
    ):
    ...
    test_tokens = tokenizer("this is a test sentence")[
        "input_ids"
    ]  # 测试tokenizer是否会自动添加 eos token，
    config["data"]["added_eos_token"] = (
        test_tokens[-1] == tokenizer.eos_token_id
    )  # 用于后续判断是否自动为句子添加 eos token
    ...
    if config["train"]["ckpt"] is not None:  # 在已有的预训练模型上进行continue training
        ...
        if model_config.vocab_size != len(tokenizer):
            ...
            if (
                not resize_flag
            ):  # 当 tokenizer 和 model 的 vocab_size 不一致，且 train_config 中指定不进行 resize 操作时，抛出一个 warning
                print(
                    f"[Vocab Warning] current model_vocab_size is `{model_config.vocab_size}`, which is not equal to tokenizer vocab_size `{len(tokenizer) }`, but `resize_model_vocab_size` is not in train_config or set as False."
                )
            else:
                raw_model.resize_token_embeddings(len(tokenizer))
                print(
                    f"Resize model vocab_size from {model_config.vocab_size} to {len(tokenizer)}."
                )
    else:
    ...
    if config["train"].get(
        "train_token_embeddings_only", False
    ):  # 是否只训练 token embedding
        print("[Train Token Embedding Only] Start to freeze model...")
        token_embedding_layer_name = config["train"].get(
            "token_embedding_layer_name", ""
        )
        assert (
            token_embedding_layer_name
        ), "[Train Token Embedding Only] Can not find `token_embedding_layer_name` in train config, or you can set `train_token_embeddings_only` to False."

        from utils.model_freeze import freeze_model_exclude_token_embeddings

        freeze_model_exclude_token_embeddings(raw_model, token_embedding_layer_name)
    ...
    if config["train"].get("use_lora", False):
        ...
        assert (
            target_modules
        ), "You must specify `target_modules` since you want to use LoRA."
