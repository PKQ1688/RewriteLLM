#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time         : 2023/9/25 16:30
# @Author       : adolf
# @Email        : adolf1321794021@gmail.com
# @LastEditTime : 2023/9/25 16:30
# @File         : trainer.py
import os
import time
import shutil
...
from TrainModel.solver.evaluator import *
...
class Trainer:
    ...
    def get_lr_scheduler(self):
        self.scheduler = get_cosine_schedule_with_warmup(
            ...
            num_warmup_steps=int(
                self.config["train"]["num_warmup_steps"] * self.lr_scheduler_factor
            ),
            num_training_steps=int(
                self.config["train"]["num_training_steps"] * self.lr_scheduler_factor
            ),
        )
    ...
    def save_model(self):
        ...
        if self.accelerator.is_main_process:
            ...
            while len(os.listdir(self.work_dir)) >= self.config["train"].get(
        ...
        stage3_save_16bit_model = os.environ.get(
            "ACCELERATE_DEEPSPEED_ZERO3_SAVE_16BIT_MODEL", "false"
        )
        ...
        if stage3_save_16bit_model in ["true", "True"]:
            ...
            raise ValueError(
                f"`stage3_save_16bit_model` in accelerate config should be a bool str, while reveived: {stage3_save_16bit_model}."
            )
        ...
        if deep_speed_stage == "3":  # Stage3 需要走单独存储的方式
            if stage3_save_16bit_model:
                ...
                cur_save_dir = os.path.join(
                    self.work_dir, f"checkpoint-{self.data_step}"
                )
                ...
                self.accelerator.print(
                    f"[+] Stage {deep_speed_stage}: Checkpoint has saved at {cur_save_dir}."
                )
            else:
                ...
                self.accelerator.print(
                    f'[+] Stage 3: Checkpoint has saved at {os.path.join(self.work_dir, f"checkpoint-{self.data_step}")}.'
                )
            ...
            if self.accelerator.is_main_process:
                ...
                self.accelerator.print(
                    f"Save ckpt used: {round(time.time() - start, 2)}s."
                )
        ...
        with tqdm(total=self.config["train"]["num_training_steps"]) as pbar:
            while True:
                ...
                for batch in train_loader:
                    ...
                    if self.data_step % self.eval_interval == 0 and self.train_and_eval:
                        self.accelerator.wait_for_everyone()  # stage 3 needs all model params on all devices
                        self.eval()

                    if self.data_step % self.save_interval == 0:
                        self.save_model()
                ...
                self.epoch += 1
                self.accelerator.print(
                    f"Start {self.epoch - 1} -> {self.epoch} epoch..."
                )

    def local_log(self, loss_dict_list):
        """
        log to local file.
        ...
        self.writer.add_scalar(
            f"Training/Token per second per gpu", token_per_second, self.data_step
        )
        self.accelerator.print(f"token per second per gpu: {token_per_second}")

        for k, values in loss_dict_list.items():
            self.writer.add_scalar(
                "Training/{}".format(k), sum(values) / len(values), self.data_step
            )
            self.accelerator.print(f"{k}: {sum(values) / len(values)}")

        current_lr = self.optim.param_groups[0]["lr"]

        self.writer.add_scalar("Training/LR", current_lr, self.data_step)
        ...
        self.writer.add_scalar(
            "Training/Trained tokens per gpu(B)", current_tokens, self.data_step
        )
        self.accelerator.print(f"trained tokens per gpu(B): {current_tokens}")

        if self.optim.scaler is not None:
            self.writer.add_scalar(
                "Training/Loss Scale", self.optim.scaler.get_scale(), self.data_step
            )
    ...
    def eval(self):
        ...
        if not eval_methods:
            raise ValueError(
                f"You must specify eval_methods in {self.all_eval_methods}, otherwise you could set `train_and_eval` to False."
            )
        ...
        for method in eval_methods:
            if method not in self.all_eval_methods:
                raise ValueError(
                    f"No valid eval_method called {method}, which should in {self.all_eval_methods}."
                )
            ...
            if method == "nll":
                ...
                if self.accelerator.is_main_process:
                    for k, v in nll_loss_dict.items():
                        self.writer.add_scalar(
                            "eval/NLL/{}".format(k), v, self.data_step
                        )
                ...
                if self.accelerator.is_main_process:
                    ...
                    cur_save_file = os.path.join(
                        eval_save_path, f"step_{self.data_step}.json"
                    )
                    json.dump(
                        res_list,
                        open(cur_save_file, "w", encoding="utf8"),
                        ensure_ascii=False,
                    )
                    self.accelerator.print(
                        f"[+] Eval Performance has been saved at {cur_save_file}."
                    )
                ...
                if self.accelerator.is_main_process:
                    ...
                    for k, v in acc_dict.items():
                        self.writer.add_scalar(
                            "eval/SC-Acc/{}".format(k), v, self.data_step
                        )
