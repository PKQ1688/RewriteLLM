#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time         : 2023/10/11 17:27
# @Author       : adolf
# @Email        : adolf1321794021@gmail.com
# @LastEditTime : 2023/10/11 17:27
# @File         : ppo_training_base.py
# -*- coding: utf-8 -*-
...
class ScriptArguments:
    ...
    mini_batch_size: Optional[int] = field(
        default=1, metadata={"help": "PPO minibatch size"}
    )
    max_source_length: Optional[int] = field(
        default=256, metadata={"help": "Max length of prompt input text"}
    )
    max_target_length: Optional[int] = field(
        default=256, metadata={"help": "Max length of output text"}
    )
    ...
    target_kl: Optional[float] = field(
        default=0.1, metadata={"help": "The kl target for early stopping"}
    )
    ...
    learning_rate: Optional[float] = field(
        default=1.5e-5, metadata={"help": "Learning rate"}
    )
    ...
    output_dir: Optional[str] = field(
        default="outputs-rl", metadata={"help": "The output directory"}
    )
    ...
    def __post_init__(self):
        ...
        if self.reward_model_name_or_path is None:
            raise ValueError(
                "You must specify a valid reward_model_name_or_path to run training."
            )
...
def main():
    ...
    if args.do_train:
        ...
        for step, batch in tqdm(enumerate(trainer.dataloader)):
            ...
            question_tensors = [
                torch.LongTensor(i).to(device).squeeze(0) for i in question_tensors
            ]
            responses = []
            response_tensors = []
            ...
            score_outputs = [
                get_reward_model_output(reward_model, reward_tokenizer, q, r, device)
