...
from GetRewriteData.base_data_read import convert_example

from LossFunction.FocalLoss import FocalLoss
from Metrics.class_metrics import ClassEvaluator

parser = argparse.ArgumentParser()
parser.add_argument(
    "--model", default="bert-base-uncased", type=str, help="backbone of encoder."
)
parser.add_argument(
    "--train_path", default=None, type=str, help="The path of train set."
)
...
parser.add_argument(
    "--num_labels", default=2, type=int, help="Total classes of labels."
)
parser.add_argument(
    "--use_class_weights",
    default=False,
...
parser.add_argument(
    ...
    default=10.0,
    type=float,
    help="max scale ratio when use class weights.",
)
parser.add_argument(
    "--loss_func",
...
parser.add_argument(
    "--focal_loss_alpha", default=0.25, type=float, help="alpha of focal loss."
)
parser.add_argument(
    "--focal_loss_gamma", default=2.0, type=float, help="gamma of focal loss."
)
    ...
    with torch.no_grad():
        for step, batch in enumerate(data_loader):
            ...
            metric.add_batch(
                ...
                gold_batch=batch["labels"].detach().cpu().numpy().tolist(),
            )
...
def train():
    ...
    if args.use_class_weights:  # 当出现imbalance情况时进行少样本类别系数补偿
        ...
        class_weight_dict = {}
        class_count_sorted_tuple = sorted(
            class_count_dict.items(), key=lambda x: x[1], reverse=True
        )
        for i in range(len(class_count_sorted_tuple)):
            if i == 0:
                ...
                scale_ratio = (
                    class_count_sorted_tuple[0][1] / class_count_sorted_tuple[i][1]
                )
                scale_ratio = min(
                    scale_ratio, args.max_scale_ratio
                )  # 数量少多少倍，loss就scale几倍
    ...
    if args.loss_func == "cross_entropy":
        ...
        criterion = FocalLoss(
            device=args.device, alpha=args.focal_loss_alpha, gamma=args.focal_loss_gamma
        )
