...
def main():
    ...
    if not model_args.model_type:
        raise ValueError(
            "Please specify a model_type, e.g. llama, chatglm, bloom, etc."
        )
    ...
    if training_args.use_peft:
        if training_args.peft_path is not None:
            ...
            model = PeftModel.from_pretrained(
                model, training_args.peft_path, is_trainable=True
            )
        else:
            logger.info("Init new peft model")
            target_modules = (
                training_args.target_modules.split(",")
                if training_args.target_modules
                else None
            )
